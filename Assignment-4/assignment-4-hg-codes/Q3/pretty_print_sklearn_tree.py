...
def pretty_print_sklearn_tree(tree_clf, feature_names):
    ...
    for i in range(n_nodes):
        ...
        if is_leaf_N[i]:
            # Check if this is a regression tree (value shape is [n_nodes, 1, 1])
            # or classification tree (value shape is [n_nodes, 1, n_classes])
            if tree_clf.tree_.value.shape[2] == 1:
                # Regression tree
                yhat = tree_clf.tree_.value[i, 0, 0]
                n_samples = tree_clf.tree_.n_node_samples[i]
                print("%s%sLeaf: yhat at this leaf = %.3f" % (
                    node_depth_N[i] * "  ", decision_str, yhat))
            else:
                # Classification tree
