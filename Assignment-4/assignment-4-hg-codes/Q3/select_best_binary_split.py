...
def select_best_binary_split(x_NF, y_N, MIN_SAMPLES_LEAF=1):
    ...
    for f in range(F):
        ...
        for v in range(V):
            # Create masks for left and right splits at this threshold
            left_mask_N = x_NF[:, f] < possib_xthresh_V[v]
            right_mask_N = np.logical_not(left_mask_N)
            
            # Get y values for left and right splits
            y_left = y_N[left_mask_N]
            y_right = y_N[right_mask_N]
            
            # Compute predictions (mean of y values)
            left_yhat_V[v] = np.mean(y_left)
            right_yhat_V[v] = np.mean(y_right)
            
            # Compute costs (sum of squared errors)
